...
#[cfg(unix)]
#[test]
fn binary_daemon_serve_accepts_socket_clients_until_stdin_closes() {
    let dir = tempdir().expect("temp dir");
    let db_path = dir.path().join("serve.sqlite3");
    let socket_path = dir.path().join("serve.sock");
    ...
    let mut child = Command::new(binary_path())
        ...
        .env("KANI_MONO_REPLAY_RESPONSE", "served answer")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    assert_eq!(value_for_key(&run, "result"), "served answer");
    ...
    assert_eq!(
        ...
        vec![
            "1 progress",
            "2 assistant_text_delta text=served answer",
            "3 final_result result=served answer",
        ]
    );
